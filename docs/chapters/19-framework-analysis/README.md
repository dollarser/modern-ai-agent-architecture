...
graph TD
    subgraph "IDE 集成型"
        Copilot[GitHub Copilot<br/>深度 IDE 集成]
        Cursor[Cursor<br/>AI-first IDE]
        Continue[Continue<br/>开源 IDE 插件]
    end

    subgraph "CLI 型"
        ClaudeCode[Claude Code<br/>终端 Agent]
    end
...
> **图 19-1：** 六大框架分类。IDE 集成型、CLI 型、SDK 型三种定位。
...
| Skills | 不将其归为本书第 12 章定义的通用 Skill Registry；应以所用 Copilot 客户端的自定义指令、Agent 或扩展文档为准。 | 推导 |
...
Claude Code 是 Anthropic 推出的终端 AI Agent，运行在命令行环境中。它是本书用于观察终端 Agent 的代表性案例：公开资料可确认其部分 Tool、MCP、Hooks、Skills 与项目指令能力，但不能据此推断它“完整实现了”本书的所有抽象组件。
...
**局限：**
- 主要交互发生在终端，是否适合团队取决于工作流、权限与可视化需求
...
**核心能力：** Agent 定义、Tool 管理、Handoff、Guardrails、Tracing
...
| Memory | Run history 可由应用显式传入，也可通过 Sessions 持久化；`context` 用于传递运行期依赖与状态。 | 官方文档 |
| MCP Client | 支持 MCP 协议，可集成 MCP Server。 | 官方文档 |
| Hooks | 通过 Lifecycle 事件（`on_handoff`、`run_hooks` 等）实现生命周期拦截。 | 官方文档 |
...
- 对 OpenAI 模型有较强依赖
- Skills 和 Hooks 系统不如 Claude Code 成熟
...
| REF-2 | [Claude Code Docs](https://docs.anthropic.com/en/docs/claude-code) | 官方文档 | Claude Code 官方文档 |
