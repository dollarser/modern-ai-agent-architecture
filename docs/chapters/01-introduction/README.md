...
```mermaid
graph TD
    LLM[LLM<br/>大语言模型] --> PE[Prompt Engineering<br/>提示工程]
    LLM --> FC[Function Calling<br/>函数调用]
    LLM --> ReAct[ReAct<br/>推理+行动]
    PE --> Instructions[Instructions<br/>全局规则]
    FC --> TC[Tool Calling<br/>工具调用]
    TC --> BT[Built-in Tools<br/>内置工具]
    TC --> MCP[MCP Tools<br/>协议工具]
    TC --> Plugins[Plugins<br/>插件工具]
    ReAct --> RP[Reasoning + Planning<br/>推理与规划]
    Instructions --> Agent[Agent Runtime<br/>运行时]
    RP --> Agent
    TC --> Agent
    Agent --> Memory[Memory<br/>记忆管理]
    Agent --> Hooks[Hooks<br/>生命周期]
    Agent --> Skills[Skills<br/>工作流模板]
```

> **图 1-2：** Agent 概念层次关系图。从 LLM 出发，三条主线（Prompt Engineering、Function Calling、ReAct）汇聚为 Agent Runtime，再派生出 Memory、Hooks、Skills 等组件。
...
graph TD
    User[User] -->|输入| Prompt
    Prompt -->|触发| Agent
    Instructions -->|约束| Agent
    Agent -->|读取| Skills
    Agent -->|调用| Tools
    Tools -->|Built-in| BT[Built-in Tools]
    Tools -->|协议| MCP[MCP Tools]
    MCP -->|提供| MCP-S[MCP Server]
    Agent -->|触发| Hooks
    Agent -->|读写| Memory
```

> **图 1-3：** 核心概念关系图。Prompt 和 Instructions 是 Agent 的输入约束，Skills 提供知识指导，Tools 提供执行能力，Hooks 管理生命周期，Memory 管理状态。
