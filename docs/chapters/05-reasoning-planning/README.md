...
> **维护边界：** 本章只定义推理、规划与重规划算法；Runtime/Hook 的生命周期控制见第 9--10 章，设计模式的组合比较见第 15 章。新增示例应链接到本章，不在第 15 章复制同一套 Planner 定义。
> **难度等级：** ⭐⭐⭐⭐
...
这些策略解决的问题不同，并不是后一种必然取代前一种的线性升级：CoT 强化中间推理，ReAct 连接推理与外部行动，ToT 探索多个候选路径，Plan-and-Execute 分离高层规划与执行。真实 Agent 可以按任务组合它们，也可能对简单任务直接回答。

> **来源类型：** 推导分析 —— 基于各论文提出的问题、机制与适用边界
...
**核心思想：** 通过中间推理步骤帮助模型分解问题。中间推理可以由模型内部完成；面向用户或日志时，通常只提供可验证的简洁依据、关键计算和引用，不应把完整隐藏推理当作必须展示的产品契约。
...
**优点：**
- 提高推理准确性，尤其对数学和逻辑问题
- 可以产出便于检查的关键步骤，但可验证性仍依赖外部计算、证据或测试
- 实现简单，不需要额外工具
...
class AdaptivePlanner(Planner):
    ...
    def replan(self, plan: Plan, failed_step: PlanStep, reason: str,
               alternatives: dict[str, str],
               available_tools: set[str],
               capability_snapshot: str) -> Plan:
        """只重写失败步骤及其依赖后继；步骤 ID 大小不代表 DAG 顺序。"""
        children: dict[int, set[int]] = {step.id: set() for step in plan.steps}
        for step in plan.steps:
            for dependency in step.depends_on:
                children.setdefault(dependency, set()).add(step.id)

        affected = {failed_step.id}
        frontier = [failed_step.id]
        while frontier:
        ...
        replacement = alternatives.get(failed_step.tool)
        if not replacement or replacement not in available_tools:
            raise ValueError(f"没有已注册的替代 Tool: {failed_step.tool}")
        ...
        for step in plan.steps:
            tool = replacement if step.id == failed_step.id else step.tool
            status = StepStatus.PENDING if step.id in affected else step.status
            result = "" if step.id in affected else step.result
            new_plan.steps.append(PlanStep(
                step.id,
                f"重规划({reason}): {step.description}" if step.id in affected else step.description,
                tool=tool,
...
已完成步骤只有在其输出仍满足新计划的输入契约、且没有未补偿副作用时才能复用。若 Capability Snapshot、权限或副作用目标变化，应拒绝自动恢复并重新审批，而不是仅替换 Tool 名。
...
2. **按依赖和风险决定是否显式规划：** 当任务存在多个依赖、昂贵或不可逆操作、需要跨角色协作，或必须支持恢复与审计时，显式计划通常更有价值；不能仅按固定步数判断。
3. **设置可校准的步骤预算：** 防止无限循环。上限应由任务基准、Tool 延迟、Token/成本预算和失败风险确定，并通过评估集持续校准，而不是照搬统一的 5--10 或 3--7 步。
...
CoT 侧重用中间步骤分解推理问题；ReAct 在决策与外部 Action/Observation 之间循环。CoT 常用于无需外部环境的推理任务，ReAct 适合需要 Tool 获取信息或改变环境的任务。两者都不要求向用户暴露完整隐藏推理；应输出足以验证结论的依据与 Tool 结果。

### Q: 如何判断规划是否合理？

好的规划应该：（1）步骤之间有清晰的依赖关系；（2）每个步骤有明确的输入、输出和完成条件；（3）粒度与风险、延迟和可恢复性匹配；（4）有错误处理与验证机制。步骤数量应由任务评估决定。

### Q: 动态重规划会不会导致无限循环？

需要设置重规划预算和终止条件，例如连续无进展、重复失败、成本超限或需要新授权。具体次数应由评估数据校准；达到预算后应返回结构化失败、请求用户决策或采用已批准的备选策略。
