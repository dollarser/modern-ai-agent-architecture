...
`step_timeout`、总超时、重试和依赖调度没有被“先声明但不生效”；它们分别在第 9 章 Runtime 和第 16 章增强版实现中加入。
...
因为 Memory、Hooks、Tool Registry、MCP 和 Plugin 在第 7 章尚未讲解。把它们提前组合会让示例与学习顺序冲突。读完相关组件后，直接使用第 16 章的增强版架构和实现更清晰。
