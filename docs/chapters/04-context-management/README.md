...
1. **预留缓冲空间：** 始终保留 20-30% 的上下文窗口作为缓冲，避免因 Tool 调用结果过大而溢出。
...
6. **Tool 结果截断：** 对过长的 Tool 结果进行截断，只保留关键信息。
...
建议截断，但保留关键信息。对于结构化结果（如 JSON），保留关键字段；对于文本结果，保留前 N 个字符或关键段落。如果结果中包含错误信息，应优先保留。
