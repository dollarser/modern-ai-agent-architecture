...
> **维护边界：** 本章是 Skill Manifest、发现、渐进式披露和安装生命周期的唯一正文来源；第 16 章只描述它如何进入 Agent Host，第 17--18 章只引用其供应链和评估要求。
...
### 1.4 Skill、Instructions、Rules、Prompt Template 与 Command

不同产品可能复用这些名称，本书按作用域和触发方式区分：

| 表面 | 主要内容 | 作用域/触发者 | 是否直接执行 |
|------|----------|---------------|----------------|
| Instructions / Rules | 持久行为、项目约定和安全要求 | Host 按组织、项目、用户、Session 分层注入 | 否；不能代替 Policy |
| Skill | 可发现、按需加载的任务工作流与资料 | Agent/Host 根据任务匹配，或用户显式指定 | 否；脚本经 Runtime/Tool |
| Prompt Template | 参数化的任务输入模板 | 应用或用户在创建 Task 时实例化 | 否 |
| Slash Command | 产品 UI 中的显式工作流入口 | 用户触发 | 自身不授权后续副作用 |
| CLI Command | 面向进程/开发者的命令接口 | 用户或受控 Tool Handler 调用 | 可能执行，必须受 Sandbox/Policy 治理 |
| Tool | 暴露给模型的结构化可调用契约 | Agent 提出调用，Runtime 执行 | 是，经 Schema、Policy、Approval |

因此 `Slash Command ≠ Skill`、`CLI Command ≠ Tool`、`Rules ≠ Policy`。同一个 `/review` 可以创建 Prompt、加载 Skill 或委派 Subagent，但这些是 Host 的映射选择，不是 Slash Command 的固有语义。
...
> **图 12-2：** Skill 在 Agent 主循环中的位置。Agent 先根据任务匹配 Skill，将命中的工作流指令加载到 Context，再进行 Reasoning 和 Planning，使 Skill 能参与计划生成；任务条件变化或重新规划时可以再次匹配。第 16 章 Agent Host 采用这一时序。
...
- **发现/验证：** Catalog 发现 Manifest，校验来源、版本、依赖、权限请求和校验和。
- **安装/启用：** 安装器原子写入隔离目录；启用只表示进入候选，不表示本次 Run 已获准。
- **加载/授权：** 匹配后加载摘要或正文；Host 再按主体、租户、Tool 和 Policy 授权。
- **执行/卸载：** Skill 指导工作流执行；任务结束或撤销后从 Context/Registry 移除，长期记忆另按保留策略处理。
...
Agent Skills 目录可以包含 `scripts/`，但“包中存在脚本”不代表 Skill 自己是执行器。脚本是供工作流引用的实现资产；是否读取、运行以及用什么权限运行，仍由 Host 决定。
...
sequenceDiagram
    ...
    participant Agent
    participant Runtime
    participant Tool as Exec / File / Domain Tool
    participant Sandbox

    Loader->>Agent: 加载 SKILL.md 与所需引用
    Agent->>Runtime: 提出结构化 Tool Call
    Runtime->>Runtime: 校验 Schema / Policy / Approval
    Runtime->>Tool: 授权后的调用
    Tool->>Sandbox: 在限定目录、网络与资源预算中执行
    Sandbox-->>Tool: stdout / artifact / exit status
    Tool-->>Runtime: 结构化 Observation
    Runtime-->>Agent: 脱敏、截断后的结果
```

> **图 12-3：** Skill 引用脚本时的安全调用链。`Skill Loader → 直接 subprocess/exec` 是禁止路径；执行必须回到 Runtime 的统一 Tool 调用面。

最低安全契约如下：

- Loader 只解析元数据、正文和被允许的引用，不在加载钩子里执行代码。
- Agent 不能把 Skill 文本视为授权；脚本请求与普通 Tool Call 使用同一套 Policy 和审批。
- Runtime 使用规范化后的 Skill 根目录解析相对路径，阻止路径穿越、越界符号链接和任意入口点。
- 命令执行使用参数数组、固定工作目录、最小环境变量、超时和输出上限，不拼接未经验证的 Shell 字符串。
- 第三方 Skill 按不可信内容处理；签名或校验和只能证明来源/完整性，不能证明脚本安全。
- Subagent 使用 Skill 时重新计算能力快照，不继承父 Agent 的全部 Tool 或秘密。

这也是 `Skill ≠ Tool` 的实践含义：Skill 可以描述“调用测试工具运行随包脚本”，但不能绕过 Runtime 自己运行该脚本。
...
本节实现用于说明运行期 Registry 的核心职责；安装、持久化、权限和依赖治理由后文 `SkillInstaller` 及独立的 `examples/skills/` 工程负责。生产环境仍需补充多进程并发、签名、租户隔离和远程来源策略。
预期输出：Skill 匹配和加载的演示
"""
...
def main():
    ...
    for task, task_type in test_cases:
        ...
        if matches:
            ...
            print(f"    → 无匹配 Skill（不加载 Skill，继续 Runtime）")
...
============================================================
  ...
  任务: '帮我写一个排序函数'
    → 无匹配 Skill（不加载 Skill，继续 Runtime）
...
### 4.2 SkillInstaller 与 SkillCatalog

`SkillRegistry` 解决运行期匹配，`SkillInstaller` 解决安装期治理，两者不能混为一个对象。本章的独立示例将 Skill 包定义为：

```text
my-skill/
├── skill.json       # name/version/keywords/dependencies/permissions
├── SKILL.md         # 匹配后加载的主指令
└── references/      # 可选，按需读取的补充资料
```

安装流程为：解析 Manifest → 校验名称与文件 → 校验 Host 授权 → 检查依赖 → 计算校验和 → 写入临时目录 → 原子替换目标目录。`.installed.json` 记录可信来源路径和 SHA-256，`SkillCatalog` 从安装目录重建索引并提供 `list/get/match`。更新复用 `install(..., replace=True)`，卸载前会拒绝仍被其他 Skill 依赖的目标。

```bash
cd examples/skills/python
python -m unittest -v test_main.py
python main.py --root .agent/skills install /path/to/my-skill
python main.py --root .agent/skills update /path/to/my-skill
python main.py --root .agent/skills list
python main.py --root .agent/skills remove my-skill
```

Python 与 TypeScript 的教学实现位于 `examples/skills/`。该示例仅安装可信本地目录，不从网络下载；Git 来源、签名、市场索引、租户作用域和沙箱属于生产扩展。安装成功也不等于可以执行任意动作：Manifest 权限只是请求，Tool 边界仍须独立授权。
...
Instructions 是较稳定的行为准则（「始终使用中文」），可按组织、项目、用户或 Session 作用域组装。Skill 是特定任务的工作流模板（「如何做代码审查」），只在匹配时加载。Instructions 由 Host 按当前作用域和策略构建，不保证原文在每轮完整重复；Skill 则按需披露。
...
| REF-4 | [Agent Skills Specification](https://agentskills.io/specification) | 开放规范 | `SKILL.md`、脚本与资源目录的通用格式 |
| REF-5 | [OpenAI Skills Catalog](https://github.com/openai/skills) | 官方开源仓库 | Codex 中 Skill 的发现、安装与分发实例 |
| REF-6 | [OpenClaw Skills](https://github.com/openclaw/openclaw/blob/main/docs/tools/skills.md) | 开源仓库文档 | Skill 作用域、允许列表、插件分发与第三方 Skill 安全边界 |
...
- [ ] 能实现 Skill 的注册、匹配和加载
- [ ] 能区分安装期 `SkillInstaller` 与运行期 `SkillRegistry`
- [ ] 能验证本地 Skill 的 Manifest、权限、依赖、来源和校验和
- [ ] 理解 Skill 组合模式
- [ ] 运行了 `examples/skills/` 的双语言契约测试
- [ ] 阅读了至少 2 篇官方参考文档
