...
**Q: Task、Run、Session、Checkpoint 和 Trace 有什么区别？**

Task 是目标，Run 是对目标的一次执行尝试，Session 是多轮交互和共享状态作用域，Checkpoint 是某个 Run 的可恢复快照，Trace 是 Run 及其子调用的因果事件链。恢复沿用未完成 Run，重试通常创建新 Run；Replay 默认不得重新产生副作用。（→ 详见第 2、9、16 章）
...
使用 SSE（Server-Sent Events）或 WebSocket 将事件推送给用户，并区分模型增量、Tool 进度、Tool 日志、审批请求、状态迁移和最终 Artifact。Tool 可以一次性返回，也可以经协议进度通知或持久化任务发布中间状态；这些事件不能伪装成最终成功。第 16 章采用可替换 `LLMAdapter`，流式能力应作为 Adapter 与 Host 输出协议的扩展，而不是假设所有 Provider 都支持相同事件格式。

**Q: 如何处理 Tool 执行中的长时间操作？**

对于耗时 Tool（如大文件处理、批量查询），可返回持久化任务 ID，并通过轮询、进度事件、SSE/回调或消息总线取得状态与最终 Artifact。契约还必须定义取消、超时、幂等、保留期和恢复语义；“已接受任务”不能被 Runtime 当作“工作已完成”。MCP Tasks、Progress 或下游 Workflow Job 都可以承载这一模式，但具体能力必须先协商。
...
**Q: Policy、Approval、Guardrail、Hook 和 Sandbox 如何配合？**

Policy 计算 `allow/ask/deny`；Approval 持久化 `ask` 的决定；Guardrails 在输入、Tool 和输出边界执行约束；Hook 是挂载 Guard 或 Observer 的生命周期机制；Sandbox 强制限制进程实际可访问的资源。它们是纵深防御的不同层，任一层都不能替代其他层。（→ 详见第 10、16、17 章）
...
关键差异包括 Token 计算与上下文限制、Tool Calling 协议、结构化输出能力、系统指令位置、流式事件和错误语义。建议通过第 16 章定义的 `LLMAdapter` 契约隔离差异，并为每个 Provider 编写适配层；第 2 章只负责总体组件定位，不包含可直接实现的 LLM 接口。（→ 详见第 16 章「Agent Host」中的 `LLMAdapter`）
...
graph TD
    ...
    subgraph Phase2["Phase 2: 进阶 (Week 5-8)"]
        ...
        W7 --> W8["Week 8<br/>Ch15-16<br/>Patterns+Agent Host"]
    end
...
> **图 20-1：** 12 周学习路线。先建立单 Agent 最小闭环，再学习可靠运行与扩展，最后进入组装、生产、评估和框架试点。
...
Week 4: 第15-16章（设计模式 + Agent Host）
```
...
| 异步任务与 Artifact | Agent 越来越多地处理长时间运行、文件和结构化交付 | 请求、事件、结果和恢复语义必须分离 | 为 Task、Status、Artifact、取消和幂等建立统一契约 |
| 运行时供应链治理 | Skills、Plugins、MCP Server 带来可安装能力 | 来源信任、版本更新和权限撤销成为运行时问题 | 签名/校验、SBOM、隔离安装，并在每次运行重新授权 |
...
**异步任务与 Artifact 的影响：** 对长时间任务，用户界面应消费状态事件和 Artifact 引用，而不是等待单个同步响应。Runtime 必须区分“已接受”“正在执行”“等待输入”和“已完成”；重连、轮询、流式传输和推送通知都应汇聚到同一个任务状态机。Artifact 还需要独立的访问控制、校验和、保留期与来源记录。
